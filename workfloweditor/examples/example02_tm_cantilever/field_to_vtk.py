...
class field_export_to_image(mupif.Application.Application):
    def __init__(self):
        mupif.Application.Application.__init__(self)
        self.field = None
        self.step_number = 0
        self.filename_base = "VTKField"
        self.metadata.update({'Name': 'field_export_to_image',
                              'ID': 'simpleImageExport',
                              'Version_date': '1.0 1.1.2020',
                              'Inputs': [
                                  {'Name': 'field', 'Type': 'mupif.Field', 'Required': False,
                                   'Type_ID': 'mupif.FieldID.FID_Temperature'}
                              ],
    ...
    def solveStep(self, tstep, stageID=0, runInBackground=False):
        if self.field:
            ...
            self.step_number += 1
            self.field.field2Image2D(fileName='%s_%d' % (self.filename_base, self.step_number))
